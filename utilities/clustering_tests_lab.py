...
# testing_image = hue_color_match(testing_image, testing_palette_image)
...
# turn palette to LAB
source_lab_palette = skimage.color.rgb2lab(source)
target_lab_palette = skimage.color.rgb2lab(target)

print(source_lab_palette.shape, target_lab_palette.shape)

# # Plot and Cluster Source
# plt.figure(1)
# plot = sns.scatterplot(source_lab_palette, c=source, s=200)
# plt.show()
...
# Scatter plot
ax.scatter(
    source_lab_palette[:, 0],
    source_lab_palette[:, 1],
    source_lab_palette[:, 2],
    c=source,
    s=200,
)
plt.show()
...
source_cluster_labels = (
    DBSCAN(eps=5, min_samples=1, n_jobs=-1).fit(source_lab_palette[:, 1:]).labels_
)
...
plt.figure(3)
plot = sns.scatterplot(
    x=target_lab_palette[:, 1], y=target_lab_palette[:, 2], c=target, s=200
)
plt.show()

target_cluster_labels = DBSCAN(eps=1.5, min_samples=1, n_jobs=-1).fit(target_ab).labels_
...
for source_cluster_index in range(source_cluster_length):
    source_cluster_lab = skimage.color.rgb2lab(
        skimage.color.hsv2rgb(
            source_lab_palette[source_cluster_labels == source_cluster_index]
        )
    )
...
for source_cluster_index, target_cluster_index in enumerate(mapping):
    ...
    source_cluster_colors_hsv = source_lab_palette[
    ...
    for source_index, source_color in enumerate(source_cluster_colors_hsv_adj):
        ...
        for target_color_indices, target_color in enumerate(target_cluster_colors_hsv):
            weighted_source = np.array(source_color[2])
            weighted_target = np.array(target_color[2])
